...
ACMD_FUNC(whosell){
	...
	if ((item_data = item_db.searchname(item_name)) == NULL){
		clif_displaymessage(fd, "Item n�o encontrado com este nome");
		return -1;
	}
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter)) {
		if (pl_sd->vender_id ) {	 // check if player is vending
			for (j = 0; j < pl_sd->vend_num; j++) {
				...
				snprintf(atcmd_output, CHAT_SIZE_MAX, "%s - Pre�o: %s%s - Quantidade: %d - %s %d,%d - %s",
	...
	if(count > 0) {
		...
		clif_displaymessage(fd, "Ningu�m est� vendendo agora.");
...
ACMD_FUNC(chatclone)
{
	...
	if ( sd->market_clone_delay + clone_delay > (int)time(NULL) ) {
		safesnprintf( atcmd_output, CHAT_SIZE_MAX, "Voc� deve esperar %d segundo(s) antes de usar este comando novamente.", sd->market_clone_delay + clone_delay - (int)time(NULL) );
		clif_displaymessage( fd, atcmd_output );
		return -1;
	}
	if( !battle_config.market_can_anywhere ) {
		if ( !map_getmapflag(sd->bl.m, MF_TOWN)) {
			clif_displaymessage( fd, "Voc� s� pode usar @chatclone em uma cidade.");
			return -1;
		}
	}
	if ( map_getcell( sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKNOCHAT ) ) {
		clif_displaymessage( fd, "Voc� n�o pode usar @chatclone nesta �rea.");
		return -1;
	}
	if ( pc_isdead(sd) ) {
		clif_displaymessage( fd, "Voc� n�o pode criar um clone de chat enquanto estiver morto." );
		return -1;
	}
	if ( npc_isnear( &sd->bl ) ) {
		clif_displaymessage( fd, "Voc� n�o pode criar um clone de chat muito perto de um npc." );
		return -1;
	}
	if ( !message || !*message ) {
		clif_displaymessage( fd, "Sintaxe: @chatclone \"<T�tulo>\" \"<Mensagem>\"");
		return -1;
	}
	{
		...
		if ( message[0] != '\"' ) {
			clif_displaymessage( fd, "Lembre-se de que o <T�tulo> deve come�ar com aspas -> \"");
			clif_displaymessage( fd, "Sintaxe: @chatclone \"<T�tulo>\" \"<Mensagem>\"");
			return -1;
		}
		i = 1;
		...
		if ( message[i] != '\"' ) {
			clif_displaymessage( fd, "Lembre-se de que o <T�tulo> deve terminar com aspas -> \"");
			clif_displaymessage( fd, "Sintaxe: @chatclone \"<T�tulo>\" \"<Mensagem>\"");
			return -1;
		}
		temp[j] = '\0';
		...
		i++;
		if ( message[i] != '\"' ) {
			clif_displaymessage( fd, "Lembre-se de que a <Mensagem> deve come�ar com aspas -> \"");
			clif_displaymessage( fd, "Sintaxe: @chatclone \"<T�tulo>\" \"<Mensagem>\"");
			return -1;
		}
		i++;
	...
	if ( strlen( title ) < 4 ) {
		clif_displaymessage( fd, "O T�tulo deve ter mais de 4 caracteres." );
		return -1;
	}
	if ( strlen( title ) >= CHATROOM_TITLE_SIZE ) {
		safesnprintf( atcmd_output, CHAT_SIZE_MAX, "O t�tulo n�o deve exceder %d caracteres.", CHATROOM_TITLE_SIZE );
		clif_displaymessage( fd, atcmd_output );
		return -1;
	}
	if ( strlen( msg ) < 4 ) {
	...
	if(battle_config.market_clone_zenycost) {
		if(sd->status.zeny < battle_config.market_clone_zenycost) {
			safesnprintf( atcmd_output, CHAT_SIZE_MAX, "N�o � poss�vel usar o clone de chat, n�o h� zeny suficiente, � necess�rio %d zeny.", battle_config.market_clone_zenycost );
			clif_displaymessage( fd, atcmd_output );
			return -1;
		}
	}
	...
	if ( !sd->market_clone_id ) {
		clif_displaymessage( fd, "Clone de chat n�o pode ser criado." );
		return -1;
	}
	if(battle_config.market_clone_zenycost)
...
ACMD_FUNC(chatclonekill) {
	if ( !sd->market_clone_id ) {
		clif_displaymessage( fd, "Voc� ainda n�o tem um clone de chat. Digite '@chat \"<T�tulo>\" \"<Mensagem>\"' para criar um.");
		return -1;
	}
